...
# Local test stack for the object storage proxy.
#
...
cmd_proxy() {
    local jar
    jar="$(ls "$PROXY_DIR"/target/object-storage-proxy-*.jar 2>/dev/null | grep -v original | head -n1)"
    if [ -z "$jar" ]; then
    ...
    exec java \
        ...
        -Dobject-storage.s3.endpoint="$S3_ENDPOINT" \
        -Dobject-storage.s3.path-style-access=true \
        -jar "$jar"
}
