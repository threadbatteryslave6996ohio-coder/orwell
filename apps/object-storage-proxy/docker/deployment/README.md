# Object storage proxy Docker deployment

This directory holds the object storage proxy's `Dockerfile` and its `.env.example`. The
proxy is built and run as part of the whole-stack compose file at the repository
root (service `object-storage-proxy`), which also runs the auth server the proxy depends
...
```bash
cp apps/object-storage-proxy/docker/deployment/.env.example \
   apps/object-storage-proxy/docker/deployment/.env
```
...
```bash
docker compose -f docker-compose.all-services.yml up --build -d object-storage-proxy nginx
...
dependency runs from `nginx` to `object-storage-proxy` rather than the other way, so
...
```bash
docker compose -f docker-compose.all-services.yml logs -f object-storage-proxy
```
...
```bash
docker compose -f docker-compose.all-services.yml exec object-storage-proxy tail -f /app/logs/audit.log
