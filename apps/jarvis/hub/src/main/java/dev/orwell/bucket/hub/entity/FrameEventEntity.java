 ...
 * source</em>. Every pushed frame is kept, so {@code jarvis-retention-worker} is the only thing
 * bounding the table — and it is a <em>separate process</em>, so this table grows unbounded if
 * that worker is not running. Size it as retention x ingest rate x frame size, per source.
 *
 * <p>Retention wins over catch-up on purpose: {@link
 * dev.orwell.bucket.retention.worker.RetentionWorker} deletes aged rows whether or not every
 * client has read them. A client away for longer than the window resumes at the oldest surviving frame rather
...
public class FrameEventEntity implements Persistable<Long> {
    /**
     * Assigned by {@link dev.orwell.bucket.hub.FrameIdAllocator} before the row is written,
